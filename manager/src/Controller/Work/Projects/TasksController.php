...
class TasksController extends AbstractController
{
    ...
    public function revoke(Task $task, Member $member, Request $request, Executor\Revoke\Handler $handler): Response
    {
        ...
        $command = new Executor\Revoke\Command($this->getUser()->getId(), $task->getId()->getValue(), $member->getId()->getValue());
    ...
    /**
     * @Route("/{id}", name=".show", requirements={"id"=App\Annotation\Guid::PATTERN})
     * @param Task $task
     * @param Request $request
    ...
    ): Response
    {
        ...
        $typeCommand = Type\Command::fromTask($this->getUser()->getId(), $task);
    ...
    /**
     * @Route("/{id}/files/{fileId}/delete", name=".files.delete", methods={"POST"})
     * @ParamConverter("member", options={"id" = "member_id"})
     * @param Task $task
     * @param string $fileId
     * @param Request $request
     ...
     * @return Response
     */
    public function fileDelete(Task $task, string $fileId, Request $request, Files\Remove\Handler $handler): Response
    {
        ...
        $command = new Files\Remove\Command($this->getUser()->getId(), $task->getId()->getValue(), $fileId);
